...
namespace Coldairarrow.WebApi
{
    public class JwtAppService : IJwtAppService
    {
        ...
        =>  CacheHelper.SystemCache.SetCache(GetKey(token),
                " ", TimeSpan.FromMinutes(Convert.ToDouble(ConfigHelper.GetValue("Jwt:ExpireMinutes")))
,ExpireType.Absolute);
        ...
        => CacheHelper.SystemCache.GetCache(GetKey(token)) == null;

        /// <summary>
        /// 判断当前 Token 是否有效
        /// </summary>
        /// <returns></returns>
        public bool IsCurrentActiveTokenAsync()
        ...
        /// <returns></returns>
        public JwtAuthorizationDto RefreshAsync(string token, Base_User dto)
        {
